...
function App({ userEmail: userEmailProp }: AppProps = {}) {
  ...
  const handleDungeonCombatStart = (enemies: Enemy[]) => {
    ...
    // Apply shrine buffs to heroes before combat
    if (currentDungeon) {
      const activeBuffs = currentDungeon.getActiveBuffs();

      // Apply damage buff (+10% ATK)
      if (activeBuffs.includes('damage')) {
        heroes.forEach(hero => {
          if (hero.isAlive) {
            hero.ATK = Math.floor(hero.ATK * 1.10);
          }
        });
      }

      // Apply stats buff (+10% to all stats)
      if (activeBuffs.includes('stats')) {
        heroes.forEach(hero => {
          if (hero.isAlive) {
            hero.ATK = Math.floor(hero.ATK * 1.10);
            hero.DEF = Math.floor(hero.DEF * 1.10);
            hero.SPD = Math.floor(hero.SPD * 1.10);
            hero.CRIT = Math.floor(hero.CRIT * 1.10);
          }
        });
      }
    }
    ...
    // Apply XP buff if active (+15% XP)
    if (currentDungeon?.hasActiveBuff('xp')) {
      combatEngine.xpMultiplier = 1.15;
    } else {
      combatEngine.xpMultiplier = 1.0;
    }
  ...
  const handleDungeonCombatEnd = () => {
    ...
    // Remove shrine buffs from heroes (they're floor-based, not permanent)
    const activeBuffs = currentDungeon.getActiveBuffs();
    if (activeBuffs.includes('damage')) {
      heroes.forEach(hero => {
        if (hero.isAlive) {
  ...
  const handleDungeonVictoryContinue = () => {
    ...
    // Use elite rewards for elite/miniboss rooms, otherwise use combat engine loot
    if (isEliteRoom && currentRoom.eliteRewards) {
      // Apply gold buff if active (+20% gold)
      let goldAmount = currentRoom.eliteRewards.gold;
      if (currentDungeon?.hasActiveBuff('gold')) {
        goldAmount = Math.floor(goldAmount * 1.20);
      }

      inventory.gold += goldAmount;
      currentRoom.eliteRewards.items.forEach(item => {
        inventory.addItem(item);
      });
      // Add to dungeon stats
      currentDungeon?.addLootToStats(goldAmount, currentRoom.eliteRewards.items.length);
    } else if (combatEngine.lootReward) {
      // Apply gold buff if active (+20% gold)
      let goldAmount = combatEngine.lootReward.gold;
      if (currentDungeon?.hasActiveBuff('gold')) {
        goldAmount = Math.floor(goldAmount * 1.20);
      }
      ...
      // Add to dungeon stats
      currentDungeon?.addLootToStats(goldAmount, combatEngine.lootReward.items.length);
    }
  ...
  return (
    <>
      <div className="container">
        <header>
          <div style={{ ...flexBetween }}>
            ...
            <div style={{ ...flexColumn, alignItems: 'flex-end', gap: SPACING[2] }}>
              <span style={{ fontSize: FONT_SIZE.sm, color: COLORS.text.muted }}>{userEmail}</span>
              <button
      ...
      {uiMode === 'worldmap' && (
        <>
          <div style={{ textAlign: 'center', padding: SPACING[5], background: COLORS.background.elevated, borderRadius: SPACING[2.5], marginBottom: SPACING[5] }}>
            <button
      ...
      <div className="equipment-grid">
        ...
        <div className="equipment-panel">
          ...
          <div className="hero-info">
            <h2 style={{ margin: 0, marginBottom: SPACING[2.5] }}>
          ...
          <div className="set-bonuses">
            ...
            {selectedHero.equipment?.getActiveSetBonuses().length === 0 ? (
              <p style={{ color: COLORS.text.tertiary, fontSize: '0.9em' }}>No active set bonuses</p>
            ) : (
          ...
          {/* Equipment Actions */}
          <div style={{ marginTop: SPACING[3.5], display: 'flex', gap: SPACING[2.5], flexWrap: 'wrap' }}>
        ...
        <div className="inventory-panel">
          ...
          <div className="inventory-grid">
            {inventory.getFilteredItems().length === 0 ? (
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: SPACING[10], color: COLORS.text.tertiary }}>
      ...
      {/* Save/Load Panel */}
      <div className="testing-panel" style={{ marginBottom: SPACING[5] }}>
        <h3>💾 Save / Load Game</h3>
        <div style={{ marginBottom: SPACING[3.5] }}>
          <label htmlFor="saveName" style={{ display: 'block', marginBottom: SPACING[1.25], fontWeight: FONT_WEIGHT.bold }}>
            Save Name:
          </label>
          <input
            id="saveName"
        ...
        <div style={{ marginTop: SPACING[2.5], fontSize: '0.85em', color: COLORS.text.disabled, fontStyle: 'italic' }}>
      ...
      <div className="testing-panel" style={{ marginBottom: SPACING[5] }}>
        ...
        {/* Combat Mode Toggle */}
        <div style={{ marginBottom: SPACING[3.5] }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: SPACING[2.5], cursor: 'pointer' }}>
            <input
              type="checkbox"
              checked={isManualMode}
              onChange={(e) => setIsManualMode(e.target.checked)}
              style={{ cursor: 'pointer', width: SPACING[5], height: SPACING[5] }}
            />
            <span style={{ fontSize: FONT_SIZE.base }}>
        ...
        <p style={{ marginTop: SPACING[2.5], fontSize: '0.9em', color: COLORS.text.muted }}>
      ...
      {combatActive && (
        <div className="combat-container" style={{ marginTop: SPACING[5] }}>
          <div className="combat-grid">
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ fontSize: '2em', marginBottom: SPACING[1.25] }}>💰</div>
                        <div style={{ fontSize: '1.5em', fontWeight: FONT_WEIGHT.bold, color: COLORS.accent.bronze }}>
                    ...
                    <div style={{
                      ...
                      {actualLoot.items.map((item: any, index: number) => (
                        ...
                        >
                          <div style={{ fontSize: '2em', marginBottom: SPACING[1.25] }}>{item.icon}</div>
                          <div style={{
                            fontSize: '0.9em',
                            fontWeight: FONT_WEIGHT.bold,
                            color: item.getRarityColor(),
                            marginBottom: BORDER_RADIUS.sm
                          }}>
                            {item.name}
                          </div>
                          <div style={{ fontSize: '0.75em', color: COLORS.text.disabled }}>
                            {item.getRarityDisplayName()} | Lv.{item.level}
                          </div>
                          <div style={{
                            fontSize: '0.75em',
                            color: COLORS.accent.brightBlue,
                    ...
                    {actualLoot.items.length > 0 && (
                      <div style={{ display: 'flex', gap: SPACING[2.5], marginTop: SPACING[3.5] }}>
                        <button
                          className="btn btn-success"
                          onClick={() => {
                    ...
                    >
                      <div className="character-header">
                        <span
                          ...
                          style={enemy.type !== 'normal' ? { color: COLORS.text.primary } : {}}
                        >
                          {enemy.type === 'boss' && '💀 '}
                          {enemy.type === 'elite' && '⭐ '}
                          {enemy.name}
            ...
            <div className="combat-controls">
              ...
              {waitingForInput && activeCharacter && (
                <div style={{ marginBottom: SPACING[5], padding: SPACING[3.5], background: '#fff3cd', borderRadius: BORDER_RADIUS.lg, border: '2px solid #ffc107' }}>
                  ...
                  {/* Skills */}
                  <div style={{ marginBottom: SPACING[3.5] }}>
                    <div style={{ fontWeight: FONT_WEIGHT.bold, marginBottom: SPACING[2], fontSize: '0.9em' }}>Skills:</div>
                    <div style={{ ...flexColumn, gap: SPACING[1.25] }}>
                      {('getSkills' in activeCharacter) && (activeCharacter as any).getSkills().map((skill: any, index: number) => {
                        ...
                        return (
                          <div key={index} style={{ display: 'flex', gap: SPACING[1.25], alignItems: 'center' }}>
                            <button
                  ...
                  <button
                    ...
                    style={{ width: '100%', marginBottom: SPACING[2.5] }}
                  >
      ...
      {tooltip && (
        ...
        >
          <div className="tooltip-header">
            ...
            <div style={{ fontSize: '0.9em', color: COLORS.text.disabled }}>
            ...
            {tooltip.item.enchantLevel > 0 && (
              <div style={{ fontSize: '0.9em', color: '#9c27b0', fontWeight: FONT_WEIGHT.bold }}>
                +{tooltip.item.enchantLevel} Enchant
              </div>
            )}
            {tooltip.item.setName && (
              <div style={{ fontSize: '0.9em', color: '#2196f3', fontWeight: FONT_WEIGHT.bold }}>
          ...
          <div className="tooltip-actions">
            ...
            <div style={{ marginTop: SPACING[1.25], fontSize: '0.85em', fontStyle: 'italic' }}>
      ...
      {enchantingItem && (
        <div className="enchanting-panel" onClick={closeEnchantPanel}>
          <div className="enchanting-content" onClick={(e) => e.stopPropagation()}>
            <h2 style={{ color: COLORS.accent.darkBlue, marginBottom: SPACING[5] }}>✨ Enchant Item</h2>

            <div style={{ textAlign: 'center', marginBottom: SPACING[5] }}>
              <div style={{ fontSize: '3em', marginBottom: SPACING[2.5] }}>{enchantingItem.icon}</div>
              <div style={{ fontSize: '1.3em', fontWeight: FONT_WEIGHT.bold, color: enchantingItem.getRarityColor(), marginBottom: SPACING[1.25] }}>
      ...
      {lootItemModal && (
        <div className="enchanting-panel" onClick={() => setLootItemModal(null)}>
          <div className="enchanting-content" onClick={(e) => e.stopPropagation()}>
            <h2 style={{ color: '#d4af37', marginBottom: SPACING[5] }}>💰 Loot Item</h2>

            <div style={{ textAlign: 'center', marginBottom: SPACING[5] }}>
              <div style={{ fontSize: '3em', marginBottom: SPACING[2.5] }}>{lootItemModal.icon}</div>
              <div style={{ fontSize: '1.3em', fontWeight: FONT_WEIGHT.bold, color: lootItemModal.getRarityColor(), marginBottom: SPACING[1.25] }}>
                {lootItemModal.name}
              </div>
              <div style={{ color: COLORS.text.disabled }}>
            ...
            <div className="enchant-details">
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: SPACING[1.25] }}>
              ...
              <p style={{ marginTop: SPACING[2.5] }}><strong>Value:</strong> {lootItemModal.goldValue} gold</p>
            </div>
        ...
        <>
          ...
          >
            <div className="tooltip-header">
              ...
              <span style={{ fontSize: '0.9em', color: COLORS.text.disabled }}>Status Effects</span>
            </div>
            ...
            {statusTooltip.character.statusEffects && statusTooltip.character.statusEffects.length > 0 ? (
              <div style={{ marginTop: SPACING[2.5] }}>
                {statusTooltip.character.statusEffects.map((effect: any, index: number) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '0.85em', color: COLORS.text.disabled }}>
              ...
              <div style={{ padding: SPACING[2.5], color: COLORS.text.disabled, textAlign: 'center' }}>
