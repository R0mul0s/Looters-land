-- Migration: Update update_leaderboard_entry to store combat_power in all categories
-- Created: 2025-11-16
-- Description: Adds p_combat_power parameter so combat power is stored regardless of category

-- Drop old function
DROP FUNCTION IF EXISTS update_leaderboard_entry(UUID, leaderboard_category, BIGINT, TEXT, INTEGER, TEXT);

-- Create new function with combat_power parameter
CREATE OR REPLACE FUNCTION update_leaderboard_entry(
  ...
  p_combat_power INTEGER DEFAULT 0
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_combat_power INTEGER := p_combat_power;
BEGIN
  ...
  SET
    ...
    -- Always update combat_power to latest value
    combat_power = GREATEST(daily_leaderboards.combat_power, v_combat_power),
